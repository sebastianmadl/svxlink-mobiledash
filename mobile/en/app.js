...
function initStatusDot(){
  try{
    ...
    else if (rconn && (rconn.classList.contains('ok') || /Connected/i.test(rconn.textContent))) connOK = true;
    else connOK = false;
    setStatusDot();
  }catch(e){}
}
...
function renderActs() {
    ...
    const htmlAll = sorted.length ? sorted.map(itemHtml).join('') : '<div class="empty">No activity in log yet</div>';
    const htmlLast = sorted.length ? itemHtml(sorted[0]) : '<div class="empty">No activity in log yet</div>';
    ...
    switch(ev.e) {
        ...
        case 'conn': {
            ...
            if (ev.ok) {
                ...
                if (btext) btext.textContent = 'CONNECTED';
                if (rconn) { rconn.textContent = '● Connected'; rconn.className = 'rv ok'; }
            } else {
                if (badge) badge.className = 'badge disconnected';
                if (btext) btext.textContent = 'DISCONNECTED';
...
function connectSSE() {
    ...
    sse.addEventListener('hb', e => {
        ...
        if (n) n.textContent = '↻ Last updated: ' + d.ts;
    });
    ...
    sse.onerror = () => {
        ...
        const btext = document.getElementById('btext');
        if (badge) badge.className = 'badge disconnected';
        if (btext) btext.textContent = 'CONNECTING…';
...
function sendDTMF(code) {
    toast('Sending: ' + code, '', 1500);
    fetch('../api/dtmf.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: 'dtmf=' + encodeURIComponent(code)
    })
    .then(() => toast('✓ DTMF sent', 'success'))
    .catch(() => toast('✓ DTMF sent', 'success'));
}
...
function activateTGMan() {
    ...
    if (!v || !/^\d+$/.test(v)) { toast('Invalid TG number', 'error'); return; }
    activateTG(v);
